...
describe('Sistema Integrado de Gestão para Fundações de Amparo a Pesquisas', () => {
  ...
  it.only('Realiza login no sistema e cria um edital médio', () => { //Teste edital médio, se houver mais de um teste, o it.only executa apenas esse teste.
    ...
    cy.get('[data-cy="nome"]').type(
      'Grupo-11 E.M. 024/2025 ana-barbosa', //Nome do Edital
      { delay: 0 },
    ...
    cy.get('[data-cy="pesquisadorSubmeterVariasPropostas"]').check(); //Marca a opção "Definir Valor Máximo de Apoio"
    cy.get('[data-cy="termo-de-aceite"]').click(); //Clica na aba de Termo de Aceite
    cy.get('[data-cy="termoDeAceite"]').click().realType('Texto do termo de aceite - Grupo 11', { delay: 0 },); //Clica no campo de Termo de Aceite

    cy.get('[data-cy=texto-do-edital]').click(); //Clica na aba Texto do Edital para seguir para a página de Texto do Edital
    cy.get('[data-cy="texto"]').click().realType('Texto do edital medio - Grupo 11', { delay: 0 },); //Preenche o campo "Texto do edital" com um texto padrão
    ...
    cy.get('[data-cy="chamadaUnsaved.termino"]').type(
      getCurrentDateTime({ addMonths: 1 }),
